...
class CommentModelTest(unittest.TestCase):
    def setUp(self):
        self.user_Kigensky = User(username='kigz', password='eee', email='test@test.com')
        self.new_blog = Blog(id=1, title='Test', content='This is a test blog', user_id=self.user_Kigensky.id)
        self.new_comment = Comment(id=1, comment ='This is a test comment', user_id=self.user_Kigensky.id, blog_id = self.new_blog.id )
